...
static int nvs_lookup_cache_rebuild(struct nvs_fs *fs)
{
    ...
    while (true) {
        ...
        if (ate.id != 0xFFFF && *cache_entry == NVS_LOOKUP_CACHE_NO_ADDR && nvs_ate_valid(fs, &ate)) {
...
/* table of calculating crc8 */
static const uint8_t crc8_ccitt_small_table[16] = {0x00, 0x07, 0x0e, 0x09, 0x1c, 0x1b, 0x12, 0x15, 0x38, 0x3f, 0x36, 0x31, 0x24, 0x23, 0x2a, 0x2d};
...
{
    uint8_t write_block_size = fs->flash_dev->write_gran;
...
static int nvs_flash_al_wrt(struct nvs_fs *fs, uint32_t addr, const void *data, size_t len)
{
    ...
    blen = len & ~(fs->flash_dev->write_gran - 1U);
    if (blen > 0) {
        rc = fs->flash_dev->ops.write(offset, data8, blen);
        if (rc) {
            /* flash write error */
            goto end;
        }
    ...
    if (len) {
        ...
        (void)memset(buf + len, fs->flash_dev->erase_value, fs->flash_dev->write_gran - len);

        rc = fs->flash_dev->ops.write(offset, buf, fs->flash_dev->write_gran);
    }
...
static int nvs_flash_rd(struct nvs_fs *fs, uint32_t addr, void *data, size_t len)
{
    int   rc;
    off_t offset;
    ...
    rc = fs->flash_dev->ops.read(offset, data, len);
    return rc;
}
 ...
 */
static int nvs_flash_block_cmp(struct nvs_fs *fs, uint32_t addr, const void *data, size_t len)
{
    const uint8_t *data8 = (const uint8_t *)data;
    int            rc;
    ...
    block_size = NVS_BLOCK_SIZE & ~(fs->flash_dev->write_gran - 1U);
...
#endif
    rc = fs->flash_dev->ops.erase(offset, fs->sector_size);

    if (rc) {
        return rc;
    }

    if (nvs_flash_cmp_const(fs, addr, fs->flash_dev->erase_value, fs->sector_size)) {
        rc = -1;
    }
...
static int nvs_ate_valid(struct nvs_fs *fs, const struct nvs_ate *entry)
{
    ...
    if ((nvs_ate_crc8_check(entry)) || (entry->offset >= (fs->sector_size - ate_size))) {
        return 0;
    }
...
static int nvs_close_ate_valid(struct nvs_fs *fs, const struct nvs_ate *entry)
{
    ...
    if ((!nvs_ate_valid(fs, entry)) || (entry->len != 0U) || (entry->id != 0xFFFF)) {
        return 0;
    }
...
static int nvs_prev_ate(struct nvs_fs *fs, uint32_t *addr, struct nvs_ate *ate)
{
    ...
    *addr += ate_size;
    ...
    rc = nvs_ate_cmp_const(&close_ate, fs->flash_dev->erase_value);
...
static int nvs_gc(struct nvs_fs *fs)
{
    ...
    uint32_t       sec_addr, gc_addr, gc_prev_addr, wlk_addr, wlk_prev_addr, data_addr, stop_addr;
    size_t         ate_size;
...
static int nvs_startup(struct nvs_fs *fs)
{
    ...
    uint8_t  erase_value = fs->flash_dev->erase_value;

    if (fs->flash_dev->mutex.lock) {
        fs->flash_dev->mutex.lock();
    }
    ...
    if (rc) {
        ...
        while ((addr & ADDR_OFFS_MASK) < (fs->sector_size - ate_size)) {
            ...
            if (nvs_ate_valid(fs, &gc_done_ate) && (gc_done_ate.id == 0xffff) && (gc_done_ate.len == 0U)) {
                gc_done_marker = true;
                break;
            }
            addr += ate_size;
    ...
    while (fs->ate_wra > fs->data_wra) {
        ...
        fs->data_wra += fs->flash_dev->write_gran;
    }
     ...
     */
    if (((fs->ate_wra + 2 * ate_size) == fs->sector_size) && (fs->data_wra != (fs->ate_wra & ADDR_SECT_MASK))) {
        rc = nvs_flash_erase_sector(fs, fs->ate_wra);
        if (rc) {
            goto end;
        }
     ...
     * space when doing gc.
     */
    if ((!rc) && ((fs->ate_wra & ADDR_OFFS_MASK) == (fs->sector_size - 2 * ate_size))) {

        rc = nvs_add_gc_done_ate(fs);
    }

    if (fs->flash_dev->mutex.unlock) {
        fs->flash_dev->mutex.unlock();
    }
...
int nvs_mount(struct nvs_fs *fs)
{
    ...
    if (fs->flash_dev->mutex.lock_init) {
        fs->flash_dev->mutex.lock_init();
    }

    write_block_size = fs->flash_dev->write_gran;
    ...
    fs->sector_size  = fs->flash_dev->sector_size;
    fs->sector_count = fs->flash_dev->sector_count;
...
int nvs_write(struct nvs_fs *fs, uint16_t id, const void *data, size_t len)
{
     ...
     */
    if ((len > (fs->sector_size - 4 * ate_size)) || ((len > 0) && (data == NULL))) {
        return -EINVAL;
    }
...
int nvs_read_hist(struct nvs_fs *fs, uint16_t id, void *data, size_t len, uint16_t cnt)
{
    ...
    if (((wlk_addr == fs->ate_wra) && (wlk_ate.id != id)) || (wlk_ate.len == 0U) || (cnt_his < cnt)) {
        return -1;
    }
